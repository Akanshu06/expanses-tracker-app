...
const forgotPasswordRequest = require("../models/password"); // Assuming you renamed the file correctly
...
module.exports.forgotPassword = async (req, res) => {
  try {
    ...
    if (!user) {
      return res.status(404).json({ success: false, message: "localhost" });
    }
    ...
    return res
      .status(401)
      .json({ message: "Some issue in forgotPassword", success: false });
  }
};
...
exports.updatePassword = async (req, res) => {
  try {
    ...
    if (forgotPasswordRequest && !forgotPasswordRequest.active) {
      ...
      if (user) {
        ...
        bcrypt.hash(newPassword, saltRounds, async (err, hash) => {
          if (err) {
            ...
            return res
              .status(500)
              .json({ error: "Error hashing password", success: false });
          }
      ...
      return res
        .status(400)
        .json({ error: "Invalid or expired request", success: false });
    }
  } catch (error) {
    console.error(error);
