...
form.addEventListener('submit', async (event) => {
    ...
    try {
        ...
        const response = await axios.post('http:// 3.109.184.246:2000/user/addexpense', expenses, { headers: { authorization: token } });
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    axios.get(`http:// 3.109.184.246:2000/user/getexpense?page=${currentPage}`, { headers: { Authorization: token } })
        .then((response) => {
    ...
    function getProducts(page) {
        axios.get(`http:// 3.109.184.246:2000/user/getexpense?page=${page}`, { headers: { Authorization: token } })
            .then((response) => {
                displayAllExpenses(response.data.expenses); // Assuming displayAllExpenses handles all expenses
...
async function deleteFunction(id) {
    try {
        const token = localStorage.getItem('token');
        const response = await axios.delete(`http:// 3.109.184.246:2000/user/deleteexpense/${id}`, { headers: { Authorization: token } });
...
premiumBtn.onclick = async (event) => {
    const token = localStorage.getItem('token');
    const response = await axios.get('http:// 3.109.184.246:2000/purchase/premiummembership', { headers: { Authorization: token } })
    try {
        var options = {
            ...
            "handler": async function (response) {
                const res = await axios.post('http:// 3.109.184.246:2000/purchase/purchasetransactionstatus', {
...
function showLeaderboard() {
    ...
    button.onclick = async () => {
        const token = localStorage.getItem('token');
        const response = await axios.get('http:// 3.109.184.246:2000/premium/premiumFeature', { headers: { Authorization: token } });
        // console.log(response);
...
async function download() {
    ...
    try {
        ...
        const response = await axios.get('http:// 3.109.184.246:2000/user/download', { headers: { Authorization: token } });
        //console.log('===>',response);
    ...
    const URLs = await axios.get('http:// 3.109.184.246:2000/user/geturl', { headers: { Authorization: token } });
    console.log(URLs);
    if (URLs) {
