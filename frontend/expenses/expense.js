...
form.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    const response = await axios.post(
      "http://localhost:3000/user/addexpense",
      expenses,
      {
...
function getProducts(page) {
  const token = localStorage.getItem("token");
  axios
    .get(`http://localhost:3000/user/getexpense?page=${page}`, {
      headers: { Authorization: token },
    })
    .then((response) => {
...
async function deleteFunction(id) {
  const token = localStorage.getItem("token");
  try {
    await axios.delete(`http://localhost:3000/user/deleteexpense/${id}`, {
      headers: { Authorization: token },
    });
  } catch (error) {
...
premiumBtn.onclick = async (event) => {
  ...
  try {
    ...
    const options = {
      key: response.data.key_id, // Corrected from 'key_id'
...
function showLeaderboard() {
  ...
  button.onclick = async () => {
    ...
    try {
      const response = await axios.get(
        "http://localhost:3000/purchase/premiumFeature",
        { headers: { Authorization: token } }
      );
